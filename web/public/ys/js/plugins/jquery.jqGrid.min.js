	...
	from : function(source){
		...
		this._compare=function(a,b,d){
			...
			if(!_usecase && typeof a !== "number" && typeof b !== "number" ) {
				a=String(a);
				b=String(b);
                if(a.localeCompare(b) < 0){return -d;}
                if(a.localeCompare(b) > 0){return d;}
                return 0;
			}else{
                if(a<b){return -d;}
			...
			if(ts.p.pginput===true) {
				$("#"+pgcnt).on('keypress','input.ui-pg-input', function(e) {
					...
					if(key === 13) {
                        ...
                        if($(this).val() >Math.ceil((ts.p.records / Number(ts.p.rowNum)))) { //edit by l02122 SMBD00385 防止页数溢出
                            $(this).val(Math.ceil((ts.p.records / Number(ts.p.rowNum))));
                        }
